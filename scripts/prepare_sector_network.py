...
def insert_electricity_distribution_grid(n, costs):
    ...
    potential = 0.1 * 20 * pop_solar.loc[solar] #pop_solar  ## added: how did it work before?!
...
def set_temporal_aggregation(n, resolution, snapshot_weightings, opts, solver_name ):
    """
    Aggregate time-varying data to the given snapshots.
    """
    print('resultion is:' , resolution)
...
def reduce_demand(n, factor =0.1 , sector =[] , method = '') :
    carrier_names ={
       "elec": ["electricity"],
       "heat": ['urban central heat', 'rural heat', 'urban decentral heat'],
       "tran": ['land transport EV', 'land transport oil',
                ],
       "ship": ['shipping methanol','shipping oil',
                ],
       ...
       'gas for industry', 'H2 for industry', 'naphtha for industry', 'low-temperature heat for industry', 
            'industry electricity','coal for industry', 'industry methanol'],
    ...
    if method == 'down-scaling' :
        
        load_change_factor = (1-int(factor[1:])/100) if factor[0]=='p' else (1+int(factor)/100)
        n.loads_t.p_set.loc[:, [i for i in load_i if i in n.loads_t.p_set.columns]] *= load_change_factor
        n.loads.p_set.loc[[i for i in load_i if i not in n.loads_t.p_set.columns]] *= load_change_factor
        print('load for' , sector_loads, 'changed by factor of', load_change_factor)     

    if method == 'peak-shaving' :

        #load_change_factor = (1-int(factor[1:])/100) if factor[0]=='p' else (1+int(factor)/100) ## old method
        load_change_factor = (1-int(factor[1:])/100) if factor[0]=='p' else (1+int(factor)/100)  ##p10 is shaving peak until total reduction equals 10% of demand
        df = n.loads_t.p_set.loc[:, [i for i in load_i]]

        ##new method
        C = df.sum().sum()*load_change_factor # equal to constant demand reduction
        # Binary search for x
        ...
        #print('Applied ',(1-peak_change_factor)*100 ,'peak shaving for' , sector_loads, 'to achieve total load reduction of ', load_change_factor*100 , '%')

        peak = df.sum(axis =1).min() + (df.sum(axis =1).max()-df.sum(axis =1).min())*load_change_factor  ##peak load is the load above base load
        peak_index = pd.DataFrame(columns = ["percentage"],index = df.index)

        for i, x in enumerate(df.sum(axis =1)):
          if x > peak:
            per = x/peak
          else:
            per = 1
          peak_index.iloc[i] = per

        for i in df.index:
           df.loc[i] = df.loc[i]/peak_index.loc[i].values

        n.loads_t.p_set.loc[:, [i for i in load_i]] = df
        print('peak shaving of load for' , sector_loads, 'by factor of', load_change_factor) 

    if method == 'free-storage' :

        load_change_factor = (float(factor[1:])/100) if factor[0]=='p' else (float(factor)/100)
        df = n.loads_t.p_set.loc[:, [i for i in load_i if i in n.loads_t.p_set.columns]]
        areas = [0]*len(load_i)
        for i, x in enumerate(df):
           areas[i] = float(np.trapz(df[x])*load_change_factor)
        #if sector=='elec' :         ##works since dist. grid losses=0, but isn't it better at low voltage?
        #  n.madd(
        #    "Store",
        #    load_i + " peak reduction storage",
        #    bus= load_i,
        #    e_nom = areas,
        #    carrier= "AC",
        #    e_cyclic = True,
        ...
        n.madd(
            "Store",
            load_i + " peak reduction storage",
            bus= n.loads.bus[load_i].values,
            e_nom = areas,
            carrier= n.buses.carrier[n.loads.bus[load_i]].values,  ##bus carrier not the load's carrier
            e_cyclic = True,
           )    
        print('peak shaving of load for' , sector_loads, 'with free storage equal to', load_change_factor, 'of demand') 

    if method == 'load-shedding' :

        load_shedding_cost = float(factor[1:]) ##Eur/MWh
        n.add("Carrier", "load_"+sector, color="#dd2e23", nice_name=sector + " Load shedding")

        n.madd(
            "Generator",
            n.loads.bus[load_i].values,
            " load shedding",
            bus=n.loads.bus[load_i].values,
            carrier="load_"+sector,
            marginal_cost=load_shedding_cost,  # Eur/MWh
            p_nom_extendable = True , 
        )
        print('added load shedding for' , sector_loads, 'with cost of', load_shedding_cost, 'EUr/MWh') 
...
if __name__ == "__main__":
    ...
    if options["transport"] and 'T' in opts:
        add_land_transport(n, costs)

    if options["heating"] and 'H' in opts:
        add_heat(n, costs)

    if options["biomass"] and 'B' in opts:
        add_biomass(n, costs)

    if options["ammonia"] :
        add_ammonia(n, costs)

    if options["industry"] and 'I' in opts:
        add_industry(n, costs)

    if options["heating"] and 'H' in opts:
        add_waste_heat(n)

    if options["agriculture"] and 'A' in opts:  # requires H and I
    ...
    n = set_temporal_aggregation(
        n, snakemake.params.time_resolution, snakemake.input.snapshot_weightings , opts, solver_name
    )
    ...
    for o in opts:
        if 'dm' in o or 'dem' in o:
            sector = o.split('I')[1] 
            factor = o.split('I')[2]
            if 'dm' in o:    
                reduce_demand(n, factor, sector , method='down-scaling')    
            if 'pdem' in o :
                reduce_demand(n, factor, sector, method='peak-shaving')     
            if 'sdem' in o:
                reduce_demand(n, factor, sector, method='free-storage')           
            if 'ldem' in o:
                reduce_demand(n, factor, sector, method='load-shedding')
